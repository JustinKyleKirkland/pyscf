...
# Abbreviations that are not entries of the database above
_alias = {
    'h2o': 'water',
...
def _build_solvent_keys():
    """Alternative spellings of the solvent names. The database is not
    consistent in the use of spaces, hyphens and the "n-" prefix of the linear
    alkanes.
    """
    keys = {_normalize(name): name for name in solvent_db}
    for name in solvent_db:
        if name.startswith('n-'):
            keys.setdefault(_normalize(name[2:]), name)
    for abbr, name in _alias.items():
        assert name in solvent_db
        keys[abbr] = name
    return keys

_solvent_keys = _build_solvent_keys()

def resolve_solvent_name(solvent):
    '''The key of `solvent` in the solvent database.

    The solvent name is matched case-insensitively, ignoring spaces and hyphens.
    Common abbreviations (such as "DMSO" or "THF") are recognized as well.
    '''
    name = None
    if isinstance(solvent, str):
        name = _solvent_keys.get(_normalize(solvent))
    if name is None:
        raise RuntimeError(
            f'Solvent "{solvent}" is not available in the solvent database. '
